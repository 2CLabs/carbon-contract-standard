...
> 注：标准范围只涉及企业碳排和碳足迹的相关合约。暂时不考虑CCER,碳汇,碳交易等。
...
​    碳配额合约从ERC20继承,使其具有金融功能,该合约管理有相关企业的碳排管理合约地址和年度配额,以及相关企业的碳账户地址.(为以后的碳交易准备).
...
* 通过企业的后台系统拉取因子库合约中的JSON格式的碳排因子EF,拉取企业碳排管理合约中的JSON格式的碳排量AD,
* 通过企业的后台系统拉取计算公式合约中的JSON格式的计算公式FORMULA
* 通过企业的后台系统拉取JS计算代码,
* 在后台系统通过JS代码应用FORMULA,AD,EF,计算出相应的碳排量.再将碳排量上传保存至企业碳排管理合约.
...
由于计算和验证均采用离线方式，企业碳排管理合约中保存的碳排数据量AD,因子库合约中保存的因子EF,以及计算公式合约中保存的计算公式FORMULA，均采用约定的JSON格式保存. 合约内部不进行JSON格式的处理.
...
* 国标因子合约命名： `GB_${行业编号}_factor_${YYMMDD}_${预留4个字符}`
* 国标公式合约命名： `GB_${行业编号}_falmula_${YYMMDD}_${预留4个字符}`
* 国标计算代码合约命名： `GB_calculate_${YYMMDD}_${预留4个字符}`
...
"sum":{
    ...
    formula_id: "01-00",
    name: "汇总公式",
    formula_id: "E_fuel + E_electricity + E_process_1",
    factor: "01-00", //关联因子
